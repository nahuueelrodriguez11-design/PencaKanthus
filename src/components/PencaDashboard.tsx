...
export default function PencaDashboard({
  ...
  // EFECTO DE PERSISTENCIA INICIAL: Restaura de forma automática la sesión y las predicciones previas del usuario
  useEffect(() => {
    setIsMounted(true);
    try {
      ...
      if (savedUserId) {
        ...
        // Buscamos al usuario en la lista actual proveniente de la base de datos
        const found = initialUsers.find(u => u.id === uid);
        if (found) {
          setCurrentUser(found);
          console.log(`[Sesión Persistida Restaurada exitosamente] Usuario ID: ${found.id} (${found.nombre})`);
          
          // Restauramos automáticamente en los inputs visuales todas sus predicciones previas guardadas
          const initialInputs: Record<number, { golesA: string; golesB: string }> = {};
          initialPredictions.forEach(p => {
            if (p.userId === found.id) {
      ...
      console.error("Aviso al intentar restaurar sesión desde localStorage:", e);
    }
  }, [initialUsers, initialPredictions]);
