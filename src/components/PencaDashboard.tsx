...
export default function PencaDashboard({
  ...
  // FUNCIÓN ASÍNCRONA DEDICADA DE EXTRACCIÓN REAL DIRECTA DESDE NEON (Punto 6)
  const fetchAndPopulateUserPredictions = async (userId: number) => {
    console.log(`LOG CONSOLA REQUERIDO - Iniciando extracción directa de Neon para user_id: ${userId}`);
    const res = await getUserPredictionsFromNeon(userId);
    if (res.success && res.predictions) {
      console.log(`LOG CONSOLA REQUERIDO - predictions encontradas en Neon: ${res.predictions.length}`);
      
      const newInputs: Record<number, { golesA: string; golesB: string }> = {};
      res.predictions.forEach((p: any) => {
        console.log(`LOG CONSOLA REQUERIDO - match_id de cada predicción: ${p.matchId} (Goles: ${p.golesA} - ${p.golesB})`);
        newInputs[p.matchId] = { golesA: String(p.golesA), golesB: String(p.golesB) };
      });

      // Rellenamos de inmediato y de forma garantizada los inputs de la interfaz
      setPredictionInputs(prev => {
        const updated = { ...prev, ...newInputs };
        console.log("LOG DIAGNÓSTICO REQUERIDO (Punto 6) - predictionInputsState final rellenado:", updated);
        return updated;
      });
      
      // Actualizamos la lista local de predicciones para que el componente visual renderice la leyenda "Guardado"
      setPredictions(prev => {
  ...
  // EFECTO DE PERSISTENCIA INICIAL REAL: Restaura al usuario completo y sincroniza inmediatamente desde Neon (Puntos 2 y 4)
  useEffect(() => {
    setIsMounted(true);
    try {
      // Leemos el objeto del usuario completo almacenado en JSON (Punto 2)
      const savedUserJson = localStorage.getItem("kanthus_penca_session_user_full");
      if (savedUserJson) {
        const savedUser = JSON.parse(savedUserJson);
        setCurrentUser(savedUser);
        setHasRestoredSession(true);
        console.log(`[PERSISTENCIA INFALIBLE] Usuario Completo Restaurado desde localStorage: ID ${savedUser.id} (${savedUser.nombre})`);
        
        // Extraemos forzosamente desde Neon las predicciones de ese mismo user_id real (Punto 6)
        fetchAndPopulateUserPredictions(savedUser.id);
      } else {
        // Compatibilidad por si el usuario guardó solo el ID antiguo en la recarga
        ...
        if (savedUserId) {
          ...
          if (found) {
            ...
            setHasRestoredSession(true);
            console.log(`[PERSISTENCIA POR ID] Usuario ID: ${found.id} (${found.nombre})`);
            // Guardamos al instante el objeto completo en JSON para futuras recargas (Punto 1)
            localStorage.setItem("kanthus_penca_session_user_full", JSON.stringify(found));
            fetchAndPopulateUserPredictions(found.id);
          }
        }
      }
    } catch (e) {
      console.error("Aviso al intentar restaurar sesión completa:", e);
    }
  }, [initialUsers]);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (res.success && res.user) {
      ...
      setHasRestoredSession(true);
      try { 
        // PUNTO 1: Al iniciar sesión, guardar el usuario completo en localStorage
      ...
      // Disparamos la extracción en vivo de la base de datos de producción real Neon (Punto 6)
      fetchAndPopulateUserPredictions(res.user.id);
    } else {
  ...
  const loginQuickDemoUser = () => {
    ...
    if (demo) {
      ...
      // Disparamos la extracción en vivo de la base de datos real
      fetchAndPopulateUserPredictions(demo.id);
    }
  };
  ...
  const loginQuickAdminUser = () => {
    ...
    if (admin) {
      ...
      // Disparamos la extracción en vivo de la base de datos real
      fetchAndPopulateUserPredictions(admin.id);
    } else {
