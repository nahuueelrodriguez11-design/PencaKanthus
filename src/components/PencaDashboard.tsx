...
export default function PencaDashboard({
  ...
  // Estado para saber si ya hay una sesión persistida guardada en localStorage (Punto 3 y 4)
  const [hasRestoredSession, setHasRestoredSession] = useState(false);

  // FUNCIÓN UNIFICADA, SIMPLE Y ROBUSTA: loadUserPredictions(userId) (Punto 5)
  const loadUserPredictions = async (userId: number) => {
    console.log(`[loadUserPredictions] Iniciando carga unificada para userId: ${userId}`);
    const storageKey = `kanthus_predictions_user_${userId}`;
    let loadedInputs: Record<number, { golesA: string; golesB: string }> = {};
    let loadedPredsObjs: PredictionObj[] = [];

    // 1. PRIMERO: Leer y pintar predicciones instantáneamente desde localStorage (Punto 2)
    try {
      const cached = localStorage.getItem(storageKey);
      if (cached) {
        const parsedCached = JSON.parse(cached);
        loadedInputs = parsedCached.inputs || {};
        loadedPredsObjs = parsedCached.list || [];
        console.log(`[loadUserPredictions] Éxito instantáneo: Pintando ${Object.keys(loadedInputs).length} inputs desde localStorage.`);
        
        // Pintar inputs de inmediato sin esperar a la red (Punto 3)
        setPredictionInputs(prev => ({ ...prev, ...loadedInputs }));
        setPredictions(prev => {
          const filtered = prev.filter(old => old.userId !== userId);
          return [...filtered, ...loadedPredsObjs];
        });
      }
    } catch (e) {
      console.error("[loadUserPredictions] Aviso al leer caché local de predicciones:", e);
    }

    // 2. LUEGO: Hacer fetch real en segundo plano a Neon (Punto 2)
    try {
      const res = await getUserPredictionsFromNeon(userId);
      if (res.success && res.predictions && res.predictions.length > 0) {
        console.log(`[loadUserPredictions] Sincronización Neon: Recibidos ${res.predictions.length} pronósticos de la base real.`);
        
        const neonInputs: Record<number, { golesA: string; golesB: string }> = {};
        res.predictions.forEach((p: any) => {
          neonInputs[p.matchId] = { golesA: String(p.golesA), golesB: String(p.golesB) };
        });

        // Combinamos priorizando la frescura de la base de datos real
        const finalMergedInputs = { ...loadedInputs, ...neonInputs };
        
        // Actualizar inputs en el controlled state de inmediato (Punto 3)
        setPredictionInputs(prev => {
          const updated = { ...prev, ...finalMergedInputs };
          console.log("[loadUserPredictions] predictionInputsState final consolidado:", updated);
          return updated;
        });

        setPredictions(prev => {
          const filtered = prev.filter(old => old.userId !== userId);
          return [...filtered, ...res.predictions];
        });

        // 3. FINALMENTE: Si Neon devuelve datos, sincronizar y actualizar localStorage (Punto 2 y 5)
        try {
          localStorage.setItem(storageKey, JSON.stringify({
            inputs: finalMergedInputs,
            list: res.predictions
          }));
          console.log(`[loadUserPredictions] Caché local kanthus_predictions_user_${userId} resincronizada con éxito.`);
        } catch (e) {}
      }
    } catch (e) {
      console.error("[loadUserPredictions] Fallo de red con Neon, manteniendo valores instantáneos de localStorage:", e);
    }
  };

  // EFECTO DE CARGA INICIAL INQUEBRANTABLE (Punto 2 y 4)
  useEffect(() => {
    setIsMounted(true);
    try {
      // Primero restaurar el usuario desde localStorage
      const savedUserJson = localStorage.getItem("kanthus_penca_session_user_full");
      if (savedUserJson) {
        const savedUser = JSON.parse(savedUserJson);
        setCurrentUser(savedUser);
        setHasRestoredSession(true);
        console.log(`[Carga Inicial] Usuario Restaurado: ID ${savedUser.id} (${savedUser.nombre})`);
        
        // Cargar predicciones inmediatamente con la única función unificada
        loadUserPredictions(savedUser.id);
      } else {
        const savedUserId = localStorage.getItem("kanthus_penca_session_user_id");
        if (savedUserId) {
          const uid = parseInt(savedUserId, 10);
          const found = initialUsers.find(u => u.id === uid);
          if (found) {
            setCurrentUser(found);
            setHasRestoredSession(true);
            localStorage.setItem("kanthus_penca_session_user_full", JSON.stringify(found));
            loadUserPredictions(found.id);
          }
        }
      }
    } catch (e) {
      console.error("Aviso al inicializar sesión:", e);
    }
  }, [initialUsers]);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (res.success && res.user) {
      ...
      loadUserPredictions(res.user.id);
    } else {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (res.success && res.user) {
      ...
      setHasRestoredSession(true);
      try { 
        // PUNTO 1: Guardar el usuario completo en localStorage
        localStorage.setItem("kanthus_penca_session_user_full", JSON.stringify(res.user));
        localStorage.setItem("kanthus_penca_session_user_id", String(res.user.id)); 
      } catch(e){}
  ...
  const loginQuickDemoUser = () => {
    ...
    if (demo) {
      ...
      setHasRestoredSession(true);
      try { 
        localStorage.setItem("kanthus_penca_session_user_full", JSON.stringify(demo));
        localStorage.setItem("kanthus_penca_session_user_id", String(demo.id)); 
      } catch(e){}
      showToast(`Autenticado como: ${demo.nombre}`, "success");
      loadUserPredictions(demo.id);
    }
  };
  ...
  const loginQuickAdminUser = () => {
    ...
    if (admin) {
      ...
      setHasRestoredSession(true);
      try { 
        localStorage.setItem("kanthus_penca_session_user_full", JSON.stringify(admin));
        localStorage.setItem("kanthus_penca_session_user_id", String(admin.id)); 
      } catch(e){}
      showToast(`Autenticado en MODO ADMIN: ${admin.nombre}`, "success");
      loadUserPredictions(admin.id);
    } else {
  ...
  const handleSavePredictionClick = async (matchId: number) => {
    ...
    if (res.success) {
      ...
      // Actualizar lista local en memoria de inmediato
      setPredictions(prev => {
        const filtered = prev.filter(p => !(p.userId === currentUser.id && p.matchId === matchId));
        return [...filtered, newPredObj];
      });

      // PUNTO 1: Al guardar una predicción, guardarla instantáneamente también en localStorage
      try {
        const storageKey = `kanthus_predictions_user_${currentUser.id}`;
        const updatedInputs = { ...predictionInputs, [matchId]: { golesA: String(gA), golesB: String(gB) } };
        // Combinamos la lista actual local con la nueva
        const currentCached = localStorage.getItem(storageKey);
        let currentList: PredictionObj[] = [];
        if (currentCached) {
          currentList = JSON.parse(currentCached).list || [];
        }
        const filteredList = currentList.filter(p => p.matchId !== matchId);
        filteredList.push(newPredObj);

        localStorage.setItem(storageKey, JSON.stringify({
          inputs: updatedInputs,
          list: filteredList
        }));
        console.log(`[Instant Save] Caché local ${storageKey} actualizada con éxito.`);
      } catch (e) {}
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            {currentUser ? (
              <div className="flex items-center gap-3 bg-neutral-900 border border-neutral-800 px-3 py-1.5 rounded-full text-xs">
                ...
                <button 
                  onClick={() => {
                    ...
                    try { 
                      // PUNTO 5: Solo cerrar sesión cuando el usuario toca el botón "Salir"
                      localStorage.removeItem("kanthus_penca_session_user_full");
                      localStorage.removeItem("kanthus_penca_session_user_id"); 
                    } catch(e){}
              ...
              // PUNTO 3: NO mostrar "Probar jugador" ni "Admin" si existe un usuario guardado
        ...
        {currentUser && (
          <div className="bg-neutral-950 border border-[#39FF14]/40 rounded-2xl p-4 mb-8 flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              {/* DEBUG VISIBLE TEMPORAL REQUERIDO EN EL PUNTO 6 */}
              <div className="mt-2 bg-black p-2 rounded border border-neutral-800 font-mono text-[10px] text-neutral-300 flex items-center gap-4 flex-wrap">
                <span>[DEBUG SESIÓN]</span>
                <span className="text-[#39FF14]">ID Cuenta: <strong>{currentUser.id}</strong></span>
                <span>Usuario: <strong>{currentUser.nombre}</strong></span>
                <span>Predicciones en Memoria: <strong>{predictions.filter(p => p.userId === currentUser.id).length}</strong></span>
              </div>
            </div>
        ...
        <section id="predicciones" className="mb-16 scroll-mt-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {matches.map(m => {
              ...
              // RE-INYECCIÓN VISUAL GARANTIZADA: Si el usuario ya tiene la predicción pero aún no tecleó en el input local,
              // forzamos a que el controlled component asuma los goles reales descargados desde la base de datos Neon.
              const currentInput = predictionInputs[m.id] || (userPred ? { 
                golesA: String(userPred.golesA), 
                golesB: String(userPred.golesB) 
              } : { golesA: "", golesB: "" });
                    ...
                    <div>
                      {userPred ? (
                        <div className="flex items-center gap-1 text-xs">
                          ...
                          <span className="text-[#39FF14] font-bold text-[11px]">
                            {userPred.calculado ? (
