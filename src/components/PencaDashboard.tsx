...
export default function PencaDashboard({
  ...
  // FUNCIÓN ASÍNCRONA DEDICADA DE EXTRACCIÓN REAL DIRECTA DESDE NEON
  ...
  // EFECTO DE PERSISTENCIA INICIAL: Restaura la sesión y sincroniza inmediatamente desde Neon
  useEffect(() => {
    setIsMounted(true);
    try {
      ...
      if (savedUserId) {
        ...
        if (found) {
          ...
          console.log(`[Sesión Restaurada] Usuario ID: ${found.id} (${found.nombre})`);
          // Disparamos la extracción en vivo de la fuente real Neon
          fetchAndPopulateUserPredictions(found.id);
        }
      }
    } catch (e) {
      console.error("Aviso al intentar restaurar sesión:", e);
    }
  }, [initialUsers]);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (res.success && res.user) {
      ...
      try { localStorage.setItem("kanthus_penca_session_user_id", String(res.user.id)); } catch(e){}
      showToast(`¡Bienvenido de vuelta, ${res.user.nombre}!`, "success");
      // Disparamos la extracción en vivo de la base de datos de producción real Neon
  ...
  const loginQuickDemoUser = () => {
    ...
    if (demo) {
      setCurrentUser(demo);
      try { localStorage.setItem("kanthus_penca_session_user_id", String(demo.id)); } catch(e){}
  ...
  const loginQuickAdminUser = () => {
    ...
    if (admin) {
      ...
      try { localStorage.setItem("kanthus_penca_session_user_id", String(admin.id)); } catch(e){}
            ...
            {currentUser ? (
              <div className="flex items-center gap-3 bg-neutral-900 border border-neutral-800 px-3 py-1.5 rounded-full text-xs">
                ...
                <button 
                  onClick={() => {
                    setCurrentUser(null);
                    try { localStorage.removeItem("kanthus_penca_session_user_id"); } catch(e){}
