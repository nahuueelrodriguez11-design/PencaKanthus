...
export async function ensureSeedData() {
  try {
    // 1. ELIMINACIÓN FORZOSA DE PARTIDOS VIEJOS / DEMO (Mundial 2022 / Pruebas)
    // Borramos cualquier encuentro viejo que tenga equipos de la siembra anterior
    await db.delete(matches).where(
      or(
        eq(matches.equipoA, "España"),
        eq(matches.equipoA, "Francia"),
        eq(matches.equipoA, "Argentina"),
        eq(matches.equipoA, "Brasil"),
        eq(matches.equipoA, "Uruguay")
      )
    );

    // 2. VERIFICACIÓN Y SIEMBRA DE LOS CRUCES DEL MUNDIAL 2026
    const matchesCount = await db.select({ value: count() }).from(matches);
    
    // Si la tabla quedó vacía tras limpiar los viejos, insertamos el bracket oficial 2026
    ...
    // 3. ASEGURAR USUARIOS BÁSICOS DE ACCESO
